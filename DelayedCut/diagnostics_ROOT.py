...
def overlay_spectra(f, istage, imode, ipred, title):
    ...
    p1 = keep(R.TPad("p1", "p1", 0, 0.5, 1, 1))
    p1.Draw()
    p2 = keep(R.TPad("p2", "p2", 0, 0.25, 1, 0.5))
    p2.Draw()
    p3 = keep(R.TPad("p3", "p3", 0, 0, 1, 0.25))
    p3.Draw()
    ...
    p2.cd()
    h_pred2nom = keep(h_pred.Clone(h_pred.GetName() + "2nom"))
    h_pred2nom.Divide(h_nom)
    h_obs2nom = keep(h_obs.Clone(h_obs.GetName() + "2nom"))
    h_obs2nom.Divide(h_nom)
    ymin = min(h_pred2nom.GetMinimum(), h_obs2nom.GetMinimum())
    ymax = max(h_pred2nom.GetMaximum(), h_obs2nom.GetMaximum())
    print(f"1 {ymin} {ymax}")
    h_pred2nom.GetYaxis().SetRangeUser(0.9 * ymin, 1.1 * ymax)
    h_pred2nom.Draw("hist")
    h_obs2nom.Draw("same")
    h_pred2nom.GetYaxis().SetRangeUser(0.9 * ymin, 1.1 * ymax)
    h_pred2nom.Draw("hist")
    h_obs2nom.Draw("same")
    ...
    p3.cd()
    h_nom2null = keep(h_nom.Clone(h_nom.GetName() + "2null"))
    h_nom2null.Divide(h_null)
    h_pred2null = keep(h_pred.Clone(h_pred.GetName() + "2null"))
    h_pred2null.Divide(h_null)
    h_obs2null = keep(h_obs.Clone(h_obs.GetName() + "2null"))
    h_obs2null.Divide(h_null)
    ymin = min(h_nom2null.GetMinimum(), h_pred2null.GetMinimum(), h_obs2null.GetMinimum())
    ymax = max(h_nom2null.GetMaximum(), h_pred2null.GetMaximum(), h_obs2null.GetMaximum())
    print(f"1 {ymin} {ymax}")
    h_nom2null.GetYaxis().SetRangeUser(0.9 * ymin, 1.1 * ymax)
    h_nom2null.Draw("hist")
    h_pred2null.Draw("hist same")
    h_obs2null.Draw("same")
    h_nom2null.GetYaxis().SetRangeUser(0.9 * ymin, 1.1 * ymax)
    h_nom2null.Draw("hist")
    h_pred2null.Draw("hist same")
    h_obs2null.Draw("same")
